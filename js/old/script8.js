...
var lesslides = document.querySelector(".lesslides");
...
function creatFolio(el, lesphotos) {
  ...
  if (el.video == undefined) { aImg.href = `#lightbox-image-${++a}`;}
  else if (el.image == undefined) {aVid.href = `#lightbox-image-${++a}`;};
...
function creatSlider(el, lesslides) {
  let figure = createNode("figure"), 
  figcaption = createNode("figcaption");
  (img = createNode("img")),
  (videos = createNode("video")),
