...
function creatFolio(el, lesphotos) {
  ...
  img.setAttribute('data-description', `${el.image.slice(0, -4)}`);
  img.setAttribute('data-large', `img/${el.photographerId}/${el.image}`);
  videos.setAttribute('data-description', `${el.video.slice(0, -4)}`);
...
class AsyncGallery { 
  constructor(settings) {
    ...
    Object.assign(this.settings, settings); //copier les valeurs de toutes les propriétés directes

    this.gallery = null;
    this.index = 0;
    this.items = [...document.querySelectorAll(this.settings.images)]; //produire une suite de valeurs 
  ...
  get loading() {
    return !this.settings.hiddenElements.includes("loader"); //déterminer si un tableau contient une valeur
  }
